...
open class Snake(val asset: AssetManager, stage: Stage, val fieldMap: FieldMap, indexX: Int, indexY: Int, bodyCnt: Int) : Actor() {
    val bodies = mutableListOf<SnakeBody>()
    protected val bodyGroup = Group()
    val head
        get() = bodies[0]
    val tale
        get() = bodies.last()
    ...
    protected fun proceed(direction: Direction) {
        var dir1 = direction
